// FlowTransactionScheduler.cdc
// Simplified FlowTransactionScheduler for Forte Actions implementation

import "FungibleToken"
import "ViewResolver"

/// Simplified FlowTransactionScheduler for Forte Actions
access(all) contract FlowTransactionScheduler {
    ...
    /// Storage path for the shared scheduler
    access(all) let storagePath: StoragePath

    init() {
        self.storagePath = /storage/flowTransactionScheduler
        
        // Create and save the shared scheduler
        let sharedScheduler <- create SharedScheduler()
        self.account.storage.save(<-sharedScheduler, to: self.storagePath)
        
        // Publish capability to the shared scheduler
        self.account.capabilities.publish(
            self.account.capabilities.storage.issue<&SharedScheduler>(self.storagePath),
            at: /public/flowTransactionScheduler
        )
    }
}